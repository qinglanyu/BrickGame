...
void CGameManager::BeginGame()
{
    ...
    emit sigPlayAudio(ai_gamestart);
}
...
void CGameManager::slotMoveDirection(int/*CGameManager::emMoveDirection*/ moveDirection)
{
    ...
    {
    case MD_Up:
        if(m_pGame->UpKeyDown())
            emit sigPlayAudio(ai_land);
        break;
    case MD_Down:
        if(m_pGame->DownKeyDown())
            emit sigPlayAudio(ai_move);
        break;
    case MD_Left:
        if(m_pGame->LeftKeyDown())
            emit sigPlayAudio(ai_move);
        break;
    case MD_Right:
        if(m_pGame->RightKeyDown())
            emit sigPlayAudio(ai_move);
        break;
    case MD_Rotate:
        if(m_pGame->RotateKeyDown())
            emit sigPlayAudio(ai_rotate);
        break;
    case MD_Floor:
