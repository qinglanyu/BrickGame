...
void CJBtn::setStyleImg(const QString &normalImg, const QString &pressedImg)
{
    ...
    setStyleSheet(QString("CJBtn{border-image: url(%1);}").arg(m_szNormalImg));
#else
    QString style("CJBtn{border-image:url(%1);}"
                "CJBtn:hover{border-image: url(%2);}"
                "CJBtn:pressed{border-image: url(%3);}");
    setStyleSheet(style.arg(m_szNormalImg).arg(m_szNormalImg).arg(m_szPressedImg));
#endif
}
